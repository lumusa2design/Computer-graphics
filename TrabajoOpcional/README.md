...
Gestiona toda la infertaz de usuario de la *Pokédex* y el intercambio entre *Pókedex* y *Pokeballs*.
...
Empecemos a comentar las diferentes funciones:
...
Cambia la vista entre *Pokédex* y *Pokeballs*
...
```
Muestra la vista de las *Pokeballs*.
...
```
Hace la lista de los *Pokemon* disponibles.
...
Actualiza el progreso en la lista.
...
Gestiona la navegación con el Jostick en la *Pokédex*.
...
Selecciona el *Pokemon* por su número identificativo.
...
### RandomMonsterSpawnerMinDistance

Genera el Spawn en una posición aleatoria en un radio con una distancia mínima (alejada del jugador).

`RandomMonsterSpawnerMinDistance.cs`
```cs
using UnityEngine;
...
public class RandomMonsterSpawnerMinDistance : MonoBehaviour
{
    ...
    void Update()
    {
        if (!player)
        {
            if (debugLogs) Debug.LogWarning("[Spawner] player is NULL. Asigna la c�mara/CenterEyeAnchor en el inspector.");
            return;
        }

        if (monsterPrefabs == null || monsterPrefabs.Length == 0)
        {
            if (debugLogs) Debug.LogWarning("[Spawner] monsterPrefabs vac�o. Arrastra tus prefabs al array.");
            return;
        }
    ...
    bool TrySpawnOne()
    {
        ...
        if (debugLogs) Debug.LogWarning("[Spawner] No encontr� suelo v�lido. Revisa groundLayers o colliders del suelo.");
        return false;
    }
...
En este caso vamos diseccionando el código:
...
Cuenta el número de *Pokemón* vivos.
...
Genera uno de forma aleatoria mirando hacia el jugador.
...
Para depurar, crea un *Gizmo* para poder orientar al *Pokemon*.

### ReactiveReticle

Hace una retícula que se amplia y se cierra con una animación según se apunta o no a la criatura
...
Empezamos comentando las funciones propias:
```cs
    void ApplyOuterBlack()
    {
        if (!outerGraphic) return;
...
Aplica negro sobre el *sprite* del círculo exterior
...
```
En el interior pone un color, según que *Pokeball* esté seleccionada:
- *Pokeball*: rojo
- *Ultraball*: amarillo
- *Safariball*: naranja
- *Masterball*: verde

Esto es un indicativo visual de las probabilidades de acertar.
...
Comprueba si la criatura está en la retícula. Si es así hará la animación más rápida y pasará la información para el autoapuntado.


### Reticle Pulse

Crea el canvas con la retícula.
...
Además es donde finalmente se hace la animación de abrir y cerrar la retícula.
